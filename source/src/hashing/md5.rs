...
fn internal_process(ctx: &mut Context, data: &[u8]) -> i32{
...
fn update_ret(ctx: &mut Context, input: &Vec<u8>, mut ilen: usize) -> i32{
    use std::convert::TryFrom;
...
fn finish_ret(ctx: &mut Context, output: &mut Vec<u8>) -> i32{
    let mut ret = -0x006E;
...
fn ret(input: &Vec<u8>, ilen: usize, output: &mut Vec<u8>) -> i32{
    let mut ret: i32 = -0x006E;
