pub struct Context {
    total: Vec<u32>, // The number of bytes processes
    state: Vec<u32>, // The intermediate digest state
    buffer: Vec<u8>, // The data block being processes unsigned char buffer[64];
}

// Done
pub fn get_uint32_be(b: &[u8; 4]) -> u32{    
...
pub fn put_uint32_be(n: u32, b: &mut[u8]){
    ...
    // Using the below declaration throws an error - I don't understand why.

    /*    
    
    b[0] = u8::try_from(n>>24).unwrap();
    b[1] = u8::try_from(n>>16).unwrap();
    b[2] = u8::try_from(n>>8).unwrap();
    b[3] = u8::try_from(n).unwrap();
...
// Done 
pub fn zeroize_u8(a: &mut Vec<u8>){
...
pub fn zeroize_u32(a: &mut Vec<u32>){
    for i in &mut a.iter_mut(){
        *i = 0u32;
    }
}
...
pub(super) fn init(ctx: &mut Context){
...
pub(super) fn free(ctx: &mut Context){
...
pub fn clone(dst: &mut Context, src: &Context){
...
pub(super) fn starts_ret(ctx: &mut Context) -> i32{
    ctx.total[0] = 0;
    ctx.total[1] = 0;
...
pub fn internal_process(ctx: &mut Context, data: &[u8]) -> i32{
...
pub(super) fn update_ret(ctx: &mut Context, input: &Vec<u8>, mut ilen: usize) -> i32{
    use std::convert::TryFrom;
...
pub(super) fn finish_ret(ctx: &mut Context, output: &mut Vec<u8>) -> i32{
    let mut ret = -0x006E;
...
pub mod test{
    ...
    pub fn self_test() {
        ...
        let mut ctx: super::Context = super::Context{total: Vec::new(), state: Vec::new(), buffer: Vec::new()};
            ...
            */
            if i == 2 {
                ...
                for j in 0..1000{
                    ...
                    /*
                    ret = update_ret(&mut ctx, &buf, 1000);
                    if(ret != 0){
                        println!("Failed - 2.");
                    }
                    */
                ...
                /*
                ret = update_ret(&mut ctx, &sha1_test_buf[i].as_bytes().to_vec(), sha1_test_buflen[i]);
                if(ret != 0){
                    println!("Failed - 3.");
                }
                */
            ...
            // finish_ret(ctx: &mut Context, output: &mut Vec<u8>)

            assert_eq!(0, finish_ret(&mut ctx, &mut sha1sum));
            /*
            ret = finish_ret(&mut ctx, &mut sha1sum);
            if ret != 0{
                println!("Failed - 4.");
            }
            */
            ...
            // As of now, we have a fresh copy of sha1sum, which we 
            // need to compare with the first row of sha1_test_sum

            // /*memcmp(sha1sum, sha1_test_sum[i], 20) != 0*/ - implementation 

            /*
            let mut temp: i32 = 0;

            for k in 0..20{
                if sha1sum[k] != sha1_test_sum[i][k] {
                    temp = 1; break;
                }
                temp = 0;
            } 
